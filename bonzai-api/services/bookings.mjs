...
export const addBooking = async ({
	...
	const { totalPrice, totalCapacity } = await calculateRoomsTotals(
		rooms,
		nights
	);
	if (guests > totalCapacity) {
		return {
			success: false,
			message: 'Too many guests for selected rooms',
		};
	}
...
export const updateBooking = async (bookingId, updateData) => {
	...
	const updatePayload = {
		...
		checkOut: calculateCheckout(
			updateData.checkIn ?? existingBooking.checkIn,
			nights
		),
	};
